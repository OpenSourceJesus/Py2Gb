...
def gbc_script_physics_prelude() -> str:
	...
	return (
		...
		'        pos = None\n'
		'        try:\n'
		'            pos = _js13k_gbc_orig_get_collider_position(collider)\n'
		'        except:\n'
		'            pos = None\n'
		'        return _js13k_gbc_unbias_pos_for_get(pos)\n'
		'    try:\n'
		...
		'        return _name\n'
		'    out = ""\n'
		'    for i, ch in enumerate(_name):\n'
		'        if ch.isupper() and i > 0 and (not _name[i - 1].isupper()):\n'
		'            out += "_"\n'
		'        out += ch.lower()\n'
		'    return out\n'
		'def _js13k_gbc_get_sim_method(_name):\n'
		'    if not isinstance(_name, str):\n'
		'        return None\n'
		'    try:\n'
		'        direct = getattr(sim, _name)\n'
		'        if callable(direct):\n'
		'            return direct\n'
		'    except:\n'
		'        pass\n'
		'    snake = _js13k_gbc_camel_to_snake(_name)\n'
		'    if snake != _name:\n'
		'        try:\n'
		'            snake_fn = getattr(sim, snake)\n'
		'            if callable(snake_fn):\n'
		'                return snake_fn\n'
		'        except:\n'
		'            pass\n'
		'    return None\n'
		...
		'def _js13k_gbc_cast_shape_via_collider(cast_collider_fn, *args, **kwargs):\n'
		...
		'    def __init__(self, _sim):\n'
		'        self._sim = _sim\n'
		'    def castShape(self, *args, **kwargs):\n'
		'        return _js13k_gbc_cast_shape_compat(*args, **kwargs)\n'
		'    def cast_shape(self, *args, **kwargs):\n'
		'        return _js13k_gbc_cast_shape_compat(*args, **kwargs)\n'
		'    def __getattr__(self, _name):\n'
		'        fn = _js13k_gbc_get_sim_method(_name)\n'
		'        if fn is not None:\n'
		'            return fn\n'
...
def py2gba_asm(
	...
	if str(target or "").lower() == "gbc":
		try:
			from GbcPyTranspiler import compile_general_script
			compiled = compile_general_script(
				str(py_code or ""),
				base_addr = 0x150,
				symbol_prefix = str(symbol_base or "gbc_script"),
			)
			listing = list(getattr(compiled, "asm_listing", []) or [])
			byte_blob = bytes(getattr(compiled, "asm_bytes", b"") or b"")
			byte_rows = []
			for i in range(0, len(byte_blob), 16):
				chunk = byte_blob[i : i + 16]
				byte_rows.append("  ; bytes %04x: %s" % (i, " ".join(["%02x" % int(b) for b in chunk])))
			out_rows = [
				"; gbc native aot script",
				"; symbol_base=%s" % str(symbol_base or "gbc_script"),
			]
			out_rows.extend(["; " + row for row in listing])
			out_rows.extend(byte_rows)
			return "\n".join(out_rows) + ("\n" if out_rows else "")
		except Exception:
			# Fall through to py2gba tooling if native AOT import/compile fails.
			pass
...
def export_gba_py_assembly(
	...
	if init_asm_chunks or update_asm_chunks:
		...
		if script_count > 0 and not builtin_only_quit and target != "gbc":
			print(
